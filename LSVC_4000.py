...
def main():
	...
	classes=["happy","sad","surprise/fear","sad/angry"]	
	...
	dtm_train_x= feature_selection.fit_transform(dtm_train_x,train_dataset['sentiment'].astype(int))
	print(len(vect.get_feature_names()))
	dtm_test_x=vect.transform(test_dataset['content'])
	dtm_test_x=feature_selection.transform(dtm_test_x)
	...
	draw_confusionmatrix(cnf_matrix,classes,"LineearSVC_4000 feature_confusion matrix")

	#finalizing the vectorizer
	vect_file="vectorizer.sav"
	joblib.dump(vect,vect_file)

	#finalizing the feature selector
	feature_selector="feature_selector.sav"
	joblib.dump(feature_selection,feature_selector)
