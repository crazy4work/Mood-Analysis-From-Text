...
def main():
	...
	classes=["happy","sad","fear/surprise","anger/disgust"]
	...
	mul_naive_bayes.fit(dtm_train_x,train_dataset['sentiment'].astype(int))

	#evaluating the model
	train_accuracy=metrics.accuracy_score(train_dataset['sentiment'].astype(int),mul_naive_bayes.predict(dtm_train_x))
	pred_y=mul_naive_bayes.predict(dtm_test_x)
	...
	draw_confusionmatrix(cnf_matrix,classes,"NB_confusion matrix")
	...
	filename="finalize_model_NB.sav"
	joblib.dump(mul_naive_bayes,filename)
